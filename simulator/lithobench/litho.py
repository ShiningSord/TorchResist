...
# 将当前目录添加到Python的模块搜索路径中，这样可以导入当前目录下的其他模块。
# sys.path.append(".")
...
def _(cmask, kernel, scale, kernelNum):
...
if __name__ == "__main__":
    ...
    input_dir = "/mnt/home/zxwang22/.jyzhou/lithobench/lithodata/MetalSet/target"
    output_dir = "./new_simulation_1nm/"
    ...
    for png_file in png_files:
        ...
        intensity_np = printed[0].detach().cpu().numpy()
        height, width = intensity_np.shape
        print(height)
        print(width)
        ...
        intensity_np_resized = cv2.resize(intensity_np, new_size, interpolation=cv2.INTER_LINEAR)
        # output_filename = os.path.join(output_dir, os.path.basename(png_file).replace('.png', '_printed.png'))

        # img = Image.fromarray(intensity_np_resized, mode='L')
        # img.save(output_filename)

        intensity_np_resized = np.expand_dims(intensity_np_resized, axis=0)
        intensity_np_all.append(intensity_np_resized)

    intensity_np_all = np.concatenate(intensity_np_all, 0)
    print(intensity_np_all.shape)
    np.save(os.path.join(output_dir, "abbe_intensity.npy"), intensity_np_all)
