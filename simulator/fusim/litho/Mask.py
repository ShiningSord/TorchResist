...
class Mask:
    ...
    def __Mask1D2Spectrum(self, NA, Wavelength):
        ...
        bgSpectrum = torch.zeros(len(f), device=self.device)  # Background spectrum
    ...
    def __PixelMask2D2Spectrum(self, NA, Wavelength):
        ...
        if (self.Nf != PntNum_X):
            raise ValueError(
                f'error: mask data size 2 ({PntNum_X}) must be equal to Mask.Nf ({self.Nf})')
        if (self.Ng != PntNum_Y):
            raise ValueError(
                f'error: mask data size 1 ({PntNum_Y}) must be equal to Mask.Ng ({self.Ng})')
        ...
        normfy = normDfy * torch.arange(start=-(PntNum_Y - 1) / 2,
                                        ...
                                        step=1, device=self.device)
        nDFTVectorX = torch.linspace(0, PntNum_X - 1, PntNum_X, device=self.device)
        nDFTVectorY = torch.linspace(0, PntNum_Y - 1, PntNum_Y, device=self.device)
        ...
        DFTHM_12D = torch.mm(
            torch.ones((PntNum_Y, 1), dtype=torch.complex64, device=self.device),
        ...
        DFTHN_12D = torch.mm(
            ...
            torch.ones((1, PntNum_X), dtype=torch.complex64, device=self.device)
        )
