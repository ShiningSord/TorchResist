...
def Calculate2DAerialImage(source, mask, projector, lithoImage, receipe, numerics):
    ...
    for iFocus in range(len(SimulationRange)):
        ...
        intensity2D = torch.zeros(wafer_nf - 1, wafer_ng - 1, len(sourceData.Value), device=mask.device)
        ...
        for j in range(len(sourceData.Value)):
            ...
            if numerics.ImageCalculationMode == 'vector':
                obliqueRaysMatrix = torch.zeros(len(fgSquare), ExyzCalculateNumber_2D, device=mask.device)
            ...
            rho2[:] = 0
            intensityTemp = torch.zeros(wafer_nf - 1, wafer_ng - 1, device=mask.device)
            ...
            for iEM in range(ExyzCalculateNumber_2D):
                ...
                if wafer_nf == mask_nf and wafer_ng == mask_ng:
                    ...
                    ExyzFrequency = torch.zeros(wafer_nf - 1, wafer_ng - 1, device=mask.device)
                    if wafer_nf > mask_nf:
                        rangeWaferNf = torch.arange((wafer_nf - mask_nf + 2) // 2, (wafer_nf + mask_nf - 2) // 2, device=mask.device)
                        rangeMaskNf = torch.arange(0, mask_nf - 1, device=mask.device)
                    else:
                        rangeWaferNf = torch.arange(0, wafer_nf - 1, device=mask.device)
                        rangeMaskNf = torch.arange((mask_nf - wafer_nf + 2) // 2, (wafer_nf + mask_nf - 2) // 2, device=mask.device)

                    if wafer_ng > mask_ng:
                        rangeWaferNg = torch.arange((wafer_ng - mask_ng + 2) // 2, (wafer_ng + mask_ng - 2) // 2, device=mask.device)
                        rangeMaskNg = torch.arange(0, mask_ng - 1, device=mask.device)
                    else:
                        rangeWaferNg = torch.arange(0, wafer_ng - 1, device=mask.device)
                        rangeMaskNg = torch.arange((mask_ng - wafer_ng + 2) // 2, (wafer_ng + mask_ng - 2) // 2, device=mask.device)
...
def check():
    ...
    plt.imshow(result.Intensity[0].abs())
    plt.show()
